...
function loadTasks() {
	const tasksInStorage = localStorage.getItem('tasks'); // получаем строку
...
// Загружаем и отображаем задачи при старте
...
formElement.addEventListener('submit', function (evt) {
	evt.preventDefault(); // отмена перезагрузки
	const taskText = inputElement.value;
	if (taskText === "") { // проверка чтоб не было пустого задания
		return;
	}
